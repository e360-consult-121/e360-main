...
exports.getAllPreviousApplications = exports.getAllApplications = void 0;
...
const getAllApplications = async (req, res, next) => {
    ...
    try {
        ...
        if (!accessToken)
            return next(new appError_1.default("Access token missing", 401));
        const userData = jsonwebtoken_1.default.decode(accessToken);
        if (!userData?.id)
            return next(new appError_1.default("Invalid token", 403));
        // console.log(userData.id)
        const applications = await VisaApplication_1.VisaApplicationModel.find({
            userId: userData.id,
        })
            .sort({ createdAt: -1 })
        ...
        console.error(err);
        return next(new appError_1.default("Failed to fetch applications", 500));
    }
};
exports.getAllApplications = getAllApplications;
const getAllPreviousApplications = async (req, res, next) => {
    try {
        const accessToken = req.cookies.accessToken;
        if (!accessToken)
            return next(new appError_1.default("Access token missing", 401));
        ...
        console.error(err);
        return next(new appError_1.default("Failed to fetch previous applications", 500));
    }
};
exports.getAllPreviousApplications = getAllPreviousApplications;
