...
exports.getPortugalPriority = void 0;
const enums_1 = require("../types/enums/enums");
const getPortugalPriority = (data) => {
    ...
    if (data.monthlyIncomeRange) {
        if (data.monthlyIncomeRange.includes("€1,000 – €2,000") ||
    ...
    if (data.sufficientSavingsFor12Months) {
        if (data.sufficientSavingsFor12Months.includes("Yes") &&
            data.sufficientSavingsFor12Months.includes("€10,000")) {
            scores.push(3); // HIGH - Yes, at least €10,000+ in savings
        }
        else if (data.sufficientSavingsFor12Months.includes("Yes") &&
            !data.sufficientSavingsFor12Months.includes("€10,000")) {
    ...
    if (data.legalResidency) {
        if (data.legalResidency.includes("Yes") &&
    ...
    if (data.otherCitizenship) {
        if (data.otherCitizenship.includes("Yes") &&
    ...
    // 7. Housing Plan
    if (data.housingPlan) {
        if (data.housingPlan.includes("rent") ||
        ...
        else if (data.housingPlan.toLowerCase().includes("undecided") ||
    ...
    if (data.stayDuration) {
        if (data.stayDuration.includes("183") ||
    ...
    switch (data.profession) {
        case "Business Owner":
            if (data.businessOwner.annualRevenue &&
            ...
            break;
        case "Remote Worker":
            if (data.remoteWorker.monthlyIncomeFromRemoteWork &&
                parseFloat(data.remoteWorker.monthlyIncomeFromRemoteWork) >= 1000) {
                scores.push(3); // HIGH
            }
            else if (data.remoteWorker.monthlyIncomeFromRemoteWork &&
            ...
            break;
        case "Investor":
            if (data.investor.investmentAmount &&
                parseFloat(data.investor.investmentAmount) >= 200000) {
                scores.push(3); // HIGH
            }
            else if (data.investor.investmentAmount &&
            ...
            else if (data.investor.industryInterest === "No, I am open to options") {
                scores.push(2); // MEDIUM
            }
            else {
                scores.push(1); // LOW
    ...
    if (data.dependents) {
        ...
        if (!isNaN(dependentCount)) {
            ...
            if (data.dependents.toLowerCase().includes("none") ||
                data.dependents.toLowerCase().includes("0")) {
                scores.push(3); // HIGH - No dependents to support
            }
            else if (data.dependents.toLowerCase().includes("many") ||
...
exports.getPortugalPriority = getPortugalPriority;
